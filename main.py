...
COLOR_PRINCIPAL = "#A4D3EE"  # Color principal
COLOR_FONDO = "#B0E2FF"  # Color de fondo
COLOR_FONDO_TEXTO = "#A4D3EE"  # Color del texto de fondo
...
class SistemaMatriculas:
    def __init__(self, root: tk.Tk):
        ...
        self.style.configure("TLabel", background=COLOR_FONDO_TEXTO, font=("Helvetica", 12))
        ...
        self.style.configure("Header.TLabel", background=COLOR_FONDO_TEXTO, foreground=COLOR_FONDO_TEXTO)
        ...
        header_frame = ttk.Frame(root, padding="80", style="Header.TFrame")
        header_frame.pack(fill="x")

        # Título
        title_label = ttk.Label(header_frame, text="I.E Miguel Grau y Seminario", style="Title.TLabel")
        title_label.place(x=30)

        # Dirección
        address_label = ttk.Label(header_frame, text="El Porvenir del Distrito de Querecotillo, Sullana", style="Address.TLabel", font=("Arial", 15))
        ...
        user_label.place(x=190, y=80)
        self.user_entry = ttk.Entry(self.iniciar_sesion_frame)
        self.user_entry.place(x=300, y=80)

        password_label = tk.Label(self.iniciar_sesion_frame, text="Contraseña", font=("Arial", 12), bg=COLOR_FONDO)
        password_label.place(x=190, y=130)
        self.password_entry = ttk.Entry(self.iniciar_sesion_frame, show="*")
        self.password_entry.place(x=300, y=130)

        login_button = ttk.Button(self.iniciar_sesion_frame, text="Iniciar Sesión", command=lambda: self.iniciar_sesion(label_imagen))
        login_button.place(x=240, y=200)
        ...
        forgot_password_label.place(x=220, y=240)
        ...
        # Redimensionar la imagen (ajusta el tamaño como desees)
        imagen = imagen.resize((300, 180))
        # Convertir la imagen a un formato compatible con Tkinter
        self.imagen_tk = ImageTk.PhotoImage(imagen)

        # Mostrar la imagen en un widget Label, alineada a la derecha
        label_imagen = tk.Label(root, image=self.imagen_tk)
        label_imagen.place(x=550, y=300)

        self.inicializar_base_datos()
    def iniciar_sesion(self, label_imagen):
    ...
    def mostrar_alumnos(self) -> None:
        ...
        style = ttk.Style()
        style.configure("Card.TFrame", background="#B0E2FF", borderwidth=2, relief="solid")
        style.configure("Card.TLabel", background="#B0E2FF")
        style.configure("Card.Header.TLabel", font=("Arial", 16, "bold"))
        style.configure("Card.Description.TLabel", font=("Arial", 12))
        style.configure("Table.TFrame", background="#B0E2FF", borderwidth=1, relief="solid")
        style.configure("Table.TLabel", font=("Arial", 12), background="#B0E2FF")
